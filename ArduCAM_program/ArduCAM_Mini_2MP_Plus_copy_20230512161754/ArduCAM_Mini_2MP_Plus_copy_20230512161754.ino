...
void setup() {
  ...
  Serial.println("Ready for Lego Detection");
}
void loop() {
  ...
  {
    temp = Serial.read();
    String tempMessage = "ACK CMD Got temp: " + String(temp);
    String tempMessageTwo = tempMessage + "END";
    Serial.println((tempMessageTwo));
    switch (temp)
    {
      case 0:
#if defined (OV2640_MINI_2MP_PLUS)
        myCAM.OV2640_set_JPEG_size(OV2640_160x120); delay(1000);
        Serial.println(F("Got 0 from computer"));
